...
use clean_slate_linux_abi::{
    ...
    LinuxErrno, LinuxSyscallRequest, LinuxSyscallResult, PollFd, Sigaction, EFAULT, EINVAL,
    ENOTTY, SYS_ARCH_PRCTL, SYS_BRK, SYS_GETPID, SYS_IOCTL, SYS_MMAP, SYS_MUNMAP, SYS_NANOSLEEP,
    SYS_POLL, SYS_RT_SIGACTION, SYS_RT_SIGPROCMASK, SYS_SET_TID_ADDRESS, SYS_UNAME, TIOCGWINSZ,
...
fn handle_sys_arch_prctl(
    ...
    linux_mem::sys_arch_prctl(request.args[0], request.args[1], ctx.pid, ctx.instance_generation)
}
...
fn handle_sys_munmap(
    ...
    linux_mem::sys_munmap(request.args[0], request.args[1], ctx.pid, ctx.instance_generation)
}
...
fn handle_sys_poll(
    ...
    let total_bytes = nfds
        .checked_mul(USER_COPY_POLL)
        .ok_or(EINVAL)?;
