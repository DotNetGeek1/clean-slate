...
cd "$ROOT"
OUT="linux-exec-args-x86_64"
OBJ="exec-args.o"
rm -f "$OBJ" "$OUT" "${OUT}.tmp"
as --64 -o "$OBJ" exec-args.S
ld -m elf_x86_64 -static -nostdlib -no-pie --build-id=none --hash-style=sysv -z norelro -T exec-args.ld -o "${OUT}.tmp" "$OBJ"
