...
**In-flight recv/send/connect:** while a bridge slot is `InService`, the service thread is inside `handle_service_request`, not in `WAIT_WORK`. Ingress uses non-blocking `RAW_RECEIVE` (one virtio harvest attempt per call) so TCP/DNS/ARP segments for that request are processed on the same thread without a separate RX wait subop. Pending bridge slots and holder-exit events count as work for `WAIT_WORK`.
