...
M7 clients block on per-request wait keys (`0x54 << 56 | request_id`) via the #145 substrate; poll retries use `BlockedResume::RestartSyscall`. The net service blocks on `NET_SUBOP_WAIT_WORK` (key `0x55 << 56`) when the bridge queue is empty. Readiness is checked with interrupts masked immediately before registering a waiter; early wakers record pending wakes per `docs/M9_BLOCK_WAKE.md`. Request completion always wakes `0x54 | request_id` from the deterministic key (no side table).

**In-flight recv/send/connect:** while a bridge slot is `InService`, the service thread is inside `handle_service_request`, not in `WAIT_WORK`. Ingress uses non-blocking `RAW_RECEIVE` (harvests up to four virtio completions into the kernel pending ring per call) so TCP/DNS/ARP segments for that request are processed on the same thread without a separate RX wait subop. The CPL3 demux may pull up to eight kernel frames per stack `receive` when early TCP data races with ARP. Pending bridge slots and holder-exit events count as work for `WAIT_WORK`.

**Idle RX without new submits:** timer harvest enqueues frames into the kernel pending ring and calls `wake_net_service_work()` so a service sleeping in `WAIT_WORK` re-enters its loop and the next `RAW_RECEIVE` (during a subsequent request or holder-exit drain) can consume harvested data. Pure smoltcp timer/retransmit work between client submits is bounded by the LAPIC tick period (~1 ms) once a request wakes the service again.

**Interim virtio RX (no IOAPIC/MSI yet):** the LAPIC timer hook `timer_poll_net_virtio_rx` harvests RX completions into a bounded kernel pending ring. When the virtio used index is ahead of the driver’s last-consumed index, each tick drains up to four completions (no pre-check that could race with syscall harvest). Syscall paths avoid large stack frames in the timer ISR; completed frames live in the bridge pending ring until `RAW_RECEIVE` copies one into userspace. Bounded counters (`harvested`, `delivered`, `pending_drop_full`, `harvest_device_err`, `stranded_observed`) and a one-shot `[NET ] rx-diag …` line record silent loss or unconsumed used-ring slots. **Follow-up #170:** replace timer harvest with virtio-net MSI/IOAPIC RX interrupts once the platform exposes device IRQ delivery.

Serial diagnostics on service idle block: `[NET ] idle block=work … rx_avail=… rx_stats=…` plus existing `[M9.E] blocked tid=… key=…`.
